...
export function renderStationCard(opts: CardOptions): string {
  ...
  return `<div class="station-card${isPlaying ? ' playing' : ''}" data-station-id="${stationId}" role="button" tabindex="0" aria-label="${esc(station.name)}${isPlaying ? ', now playing' : ''}">
  ...
  <div class="station-card-footer">
    ...
    <button class="station-card-favorite${favActive ? ' active' : ''}" data-action="${action}" data-station-id="${stationId}" title="${favActive ? 'Remove from favorites' : 'Add to favorites'}" aria-label="${favActive ? 'Remove from favorites' : 'Add to favorites'}" aria-pressed="${favActive}">
