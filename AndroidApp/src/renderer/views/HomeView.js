...
export class HomeView extends BaseComponent {
    constructor() {
        ...
        Object.defineProperty(this, "recentHistory", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: []
        });
        Object.defineProperty(this, "favoritesCount", {
            enumerable: true,
            configurable: true,
            writable: true,
            value: 0
    ...
    async afterMount() {
        ...
        this.unsubscribers.push(this.eventBus.on('player:play', () => { this.updateNowPlaying(); this.syncPlayingState(); }), this.eventBus.on('player:pause', () => { this.updateNowPlaying(); this.syncPlayingState(); }), this.eventBus.on('player:stop', () => { this.updateNowPlaying(); this.syncPlayingState(); }));
    }
    beforeUnmount() {
    ...
    async loadData() {
        try {
            ...
            if (historyResult.success)
                this.recentHistory = historyResult.data.slice(0, 6);
            if (favResult.success)
                this.favoritesCount = favResult.data.length;
        }
        catch (_) {
    ...
    attachListeners() {
        ...
        this.querySelectorAll('.station-card').forEach(card => {
            this.on(card, 'click', (e) => {
                ...
                if (item)
                    this.playerStore.play(item.station);
            });
        });
    }
