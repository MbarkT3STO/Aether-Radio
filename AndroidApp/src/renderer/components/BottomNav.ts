...
const NAV_ITEMS: NavItem[] = [
  ...
  },
  {
    route: '/history',
    label: 'History',
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>`,
  },
  {
    route: '/custom',
    label: 'My Radios',
    icon: `<svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>`,
  },
  {
    route: '/settings',
    label: 'Settings',
...
export class BottomNav extends BaseComponent {
  ...
  constructor(props: Record<string, never>) {
    super(props)
    this.unsubRoute = this.eventBus.on('route:changed', ({ route }) => {
  ...
  protected beforeUnmount(): void {
    this.unsubRoute?.()
    this.unsubRoute = null
  }

  render(): string {
    return `<nav class="bottom-nav" role="navigation" aria-label="Main navigation">
      ${NAV_ITEMS.map(item => `
        <button class="bnav-item" data-route="${item.route}" type="button">
          ${item.icon}
  ...
  protected afterMount(): void {
    this.querySelectorAll<HTMLButtonElement>('.bnav-item').forEach(btn => {
  ...
  private updateActive(): void {
    this.querySelectorAll('.bnav-item').forEach(btn => {
      const active = btn.getAttribute('data-route') === this.currentRoute
      btn.classList.toggle('bnav-item--active', active)
