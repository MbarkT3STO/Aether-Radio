...
export class MiniPlayer extends BaseComponent {
  ...
  render(): string {
    ...
    if (!station) {
      return `
        <div class="mini-player mini-player--empty">
    ...
    return `
      <div class="mini-player" id="mini-player-root">
        ...
        <div class="mini-player-controls">
          <!-- Sleep timer -->
          <div id="mini-sleep-timer-slot"></div>
          ...
          <!-- Stop -->
          <button class="mini-player-btn" id="mini-stop-btn" title="Stop">
  ...
  private mountSleepTimer(): void {
    const slot = this.querySelector('#mini-sleep-timer-slot')
    if (!slot) return
    void this.sleepTimer.mount(slot as HTMLElement)
  }
  ...
  private attachListeners(): void {
    ...
    const stopBtn = this.querySelector('#mini-stop-btn')
    if (stopBtn) {
