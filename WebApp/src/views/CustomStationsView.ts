...
export class CustomStationsView extends BaseComponent {
  ...
  private async handleAddStation(form: HTMLFormElement): Promise<void> {
    ...
    const favicon = (data.get('favicon') as string | null)?.trim()
    if (favicon) station.favicon = favicon
  ...
  private async handleEditStation(form: HTMLFormElement, stationId: string): Promise<void> {
    ...
    const editDescription = (data.get('description') as string | null)?.trim()
    updates.description = editDescription || ''
    const editFavicon = (data.get('favicon') as string | null)?.trim()
    if (editFavicon) updates.favicon = editFavicon
