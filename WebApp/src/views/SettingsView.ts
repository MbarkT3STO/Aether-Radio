...
export class SettingsView extends BaseComponent {
      ...
      <div class="stg-card">
        ...
        <div class="stg-about">
          ...
          <div>
            ...
            <div class="stg-about-version">World radio, in your browser</div>
          </div>
        </div>
        ...
        <div class="stg-row">
          <div class="stg-row-info">
            ...
            <div class="stg-row-desc">
              Stream over 30,000 radio stations from around the world. No
              install required — just open and play.
            </div>
          </div>
        </div>
        ...
        <div class="stg-row">
          <div class="stg-row-info">
            <div class="stg-row-label">Edition</div>
            <div class="stg-row-desc">Runs entirely in your browser</div>
          </div>
          <span class="stg-data-source-link" style="cursor:default;">Web</span>
        </div>
  ...
  private renderShortcutsCard(): string {
    const shortcuts: Array<{ keys: string[]; action: string }> = [
      ...
      { keys: ['Media Play/Pause'], action: 'Play / Pause (when supported by OS)' },
    ...
    return `<div class="stg-card">
      <div class="stg-card-header">
        ...
        <div>
          ...
          <div class="stg-card-sub">Active while the tab is focused</div>
        </div>
      </div>
  ...
  private attachListeners(): void {
    ...
    if (exportBtn) {
      this.on(exportBtn, 'click', async () => {
        ...
        if (result.success) {
          const n = result.data
          this.showToast(n === 0 ? 'No favorites to export' : `Exported ${n} favorite${n !== 1 ? 's' : ''}`,
            n === 0 ? 'info' : 'success')
    ...
    if (importBtn) {
      this.on(importBtn, 'click', async () => {
        ...
        if (result.success) {
          ...
          this.showToast(
            n > 0 ? `Imported ${n} favorite${n !== 1 ? 's' : ''}` : 'No new favorites imported',
            n > 0 ? 'success' : 'info'
          )
        } else if (result.error.code !== 'CANCELLED') {
    ...
    if (clearHistoryBtn) {
      this.on(clearHistoryBtn, 'click', async () => {
        ...
        this.showToast(result.success ? 'Play history cleared' : 'Failed to clear history',
          result.success ? 'success' : 'error')
      })
    }

    const dsLink = this.querySelector<HTMLElement>('#stg-data-source-link')
    if (dsLink) this.on(dsLink, 'click', () => this.bridge.openExternal(WEBSITE_URL))

    const repoBtn = this.querySelector<HTMLElement>('#stg-open-repo')
    if (repoBtn) this.on(repoBtn, 'click', () => this.bridge.openExternal(REPO_URL))

    const reportBtn = this.querySelector<HTMLElement>('#stg-report-issue')
    if (reportBtn) this.on(reportBtn, 'click', () => this.bridge.openExternal(ISSUES_URL))

    const releasesBtn = this.querySelector<HTMLElement>('#stg-open-releases')
    if (releasesBtn) this.on(releasesBtn, 'click', () => this.bridge.openExternal(RELEASES_URL))

    const devBtn = this.querySelector<HTMLElement>('#stg-open-dev')
    if (devBtn) this.on(devBtn, 'click', () => this.bridge.openExternal(DEV_URL))

    const sponsorBtn = this.querySelector<HTMLElement>('#stg-sponsor')
    if (sponsorBtn) this.on(sponsorBtn, 'click', () => this.bridge.openExternal(FUND_URL))

    // Developer avatar — fall back to local silhouette if remote load fails
