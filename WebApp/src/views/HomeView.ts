...
export class HomeView extends BaseComponent {
  ...
  private async handleFavorite(btn: HTMLElement): Promise<void> {
    ...
    const item = this.recentHistory.find(h => h.station.id === id)
    if (!item) return
    const wasFav = this.favoritesStore.isFavorite(item.station.id)

    // Optimistic swap — no full re-render
    const root = this.querySelector('#home-content')
    if (root) updateFavoriteButton(root, item.station.id, !wasFav)
