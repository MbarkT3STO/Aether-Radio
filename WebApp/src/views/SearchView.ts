...
export class SearchView extends BaseComponent {
  ...
  private async handleFavorite(btn: HTMLElement): Promise<void> {
    ...
    // Optimistic surgical toggle — avoids re-rendering the whole grid
    const root = this.querySelector('#search-results')
    if (root) updateFavoriteButton(root, station.id, !wasFav)
